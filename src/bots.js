            ...
            bot.on('message', async (ctx) => {
                ...
                db.get('SELECT * FROM users WHERE user_id = ? AND bot_id = ? AND banned = ?', [userId, botId, 'true'], async (err, row) => {
                    ...
                    if (row) {
                        ...
                        switch (path) {
                            case 'start-change':
                                await bot.telegram.setMyCommands([{command: 'start', description: ctx.message.text}]);
                                db.run('UPDATE bots SET startMsg = ? WHERE token = ?', [ctx.message.text, row.token]);
                                await ctx.reply('تم تغيير رسالة البداية بنجاح');
                                path = 'none';
                                break;
                            case 'receipt-change':
                                db.run('UPDATE bots SET receiptMsg = ? WHERE token = ?', [ctx.message.text, row.token]);
